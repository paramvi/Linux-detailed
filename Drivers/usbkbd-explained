this doc is for the usb_kbd_irq(uki) function. So according to the analysis... kbd->new (new) is the buffer and it receives the input data 
from keypresses. Now this is parsed in the function of 'uki'. 
...
1. When these keys are pressed new[0] gets the data, instead of usual new[2]. There scancodes are listed below.
  ...
  Example:
	Usually when you press say Shift_L + a which is Capital 'a'
	what happens is the 'new' string is like as follows:
							new: 2 0 4 0 0 0 0
							
							Now this is the string that is handled in the 'uki' function. In the first loop 
							for i = 1, new[0]>>1 & 1 == true, so input_report_key reports usb_kbd_keycode[224+1] 
							to the kernel, which is 'shift'. 
...
2. Second Loop is for getting to work the non-special keys... keys which independently works. Keys like 'k', 's' etc.
   ...
   A: First IF condition 
     ...
     Example:  Carrying on with the above example     
     						...
     						For this new[2]=4. So it will check the above conditions and if both true, then input_report_key 
     						will report the usb_kbd_keycode[4] to the kernel.
